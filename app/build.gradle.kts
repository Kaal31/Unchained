...
android {
    ...
    defaultConfig {
        // Own package so this fork installs alongside stock GameNative and is never
        // overwritten/reverted by official updates. Namespace stays app.gamenative so
        // code/R/BuildConfig and component (activity-alias) class names are unchanged.
        applicationId = "app.gamenative.unchained"
...
dependencies {
    ...
    // JavaSteam
    val localBuild = true // Locally-patched JavaSteam (adds LuaTools depotKeyProvider)
    if (localBuild) {
        implementation(files("../../JavaSteam-gamenative-latest/build/libs/javasteam-1.8.0.1-23-SNAPSHOT.jar"))
        implementation(files("../../JavaSteam-gamenative-latest/javasteam-depotdownloader/build/libs/javasteam-depotdownloader-1.8.0.1-23-SNAPSHOT.jar"))
