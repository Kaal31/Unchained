...
internal fun LibraryDetailPane(
    ...
    Surface {
        ...
        } else if (libraryItem.isStoreResult) {
            // Once the game is added via SLS it's a real library entry, so swap the
            // minimal add-page for the full game-info page in place (no back-and-reopen).
            var added by remember(libraryItem.appId) { mutableStateOf(false) }
            if (added) {
                AppScreen(
                    libraryItem = libraryItem.copy(isStoreResult = false),
                ...
                app.gamenative.ui.screen.library.NonOwnedGameScreen(
                    appId = libraryItem.gameId,
                    name = libraryItem.name,
                    heroImageUrl = libraryItem.heroImageUrl,
                    headerImageUrl = libraryItem.headerImageUrl,
                    onBack = onBack,
                    onAdded = { added = true },
                )
            }
