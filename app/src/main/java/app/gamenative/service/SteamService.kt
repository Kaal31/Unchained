                ...
                val downloadJob = instance!!.scope.launch {
                    try {
                        ...
                        val depotDownloader = DepotDownloader(
                            ...
                            licenses,
                            debug = true, // TEMP: LuaTools diagnostics — surfaces depot key/manifest logs
                            ...
                            // LuaTools: supply depot keys for non-owned depots Steam refuses,
                            // so DepotDownloader can decrypt and download them.
                            depotKeyProvider = { depotId ->
                        ...
                        if (mainAppDepots.isNotEmpty()) {
                            ...
                            // For LuaTools-added depots (Steam refuses a manifest
                            // request code for non-owned apps) pin the manifest to the
                            // LuaTools-supplied gid so DepotDownloader loads the bundled
                            // <depot>_<gid>.manifest from disk instead of asking Steam.
                            // Owned depots get INVALID_MANIFEST_ID -> normal PICS lookup.
                            val mainAppManifestIds = mainAppDepotIds.map { depotId ->
                                if (app.gamenative.luatools.LuaDepotKeyProvider.hasKey(depotId)) {
                                    mainAppDepots[depotId]?.manifests?.get(branch)?.gid
                                        ?: mainAppDepots[depotId]?.manifests?.values?.firstOrNull()?.gid
                                        ?: Long.MAX_VALUE
                                } else {
                                    Long.MAX_VALUE
                                }
                            ...
                            // LuaTools: stage the bundled manifests into the exact
                            // .DepotDownloader config dir now — the install flow wipes
                            // any copy placed at add time, so we re-place from the
                            // name-independent cache right before the download starts.
                            runCatching {
                                val staged = app.gamenative.luatools.LuaToolsRepository
                                    .stageManifestsInto(appId, getAppDirPath(appId))
                                if (staged > 0) Timber.i("LuaTools: staged $staged manifest(s) pre-download")
                            }
    ...
    private fun onLicenseList(callback: LicenseListCallback) {
        ...
        scope.launch {
            db.withTransaction {
                ...
                // LuaTools apps have no real Steam ownership, so their synthetic
                // licence would be pruned here (and PICS-refreshed below), hiding
                // them from the library on every launch. Keep them.
                val luaToolsPkgIds = runCatching {
                    app.gamenative.luatools.LuaToolsPrefs.getAddedAppIds()
                }.getOrDefault(emptySet())

                val licensesToRemove = licenseDao.findStaleLicences(
                    packageIds = callback.licenseList.map { it.packageID },
                ).filter { it.packageId !in luaToolsPkgIds }
                ...
                // Get PICS information with the current license database. Skip the
                // LuaTools packages — Steam has no PICS for them and refreshing would
                // clobber the synthetic app entry.
