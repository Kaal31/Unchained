...
object GameFixesRegistry {
    ...
    fun applyFor(context: Context, appId: String, container: Container) {
        ...
        // Curated per-game fix (owned games), if one exists for this id.
        val catalogId = when (source) {
            // EPIC auto-generates the id. so we need the catalog id instead.
            GameSource.EPIC -> EpicService.getEpicGameOf(gameId.toInt())?.catalogId
            else -> gameId
        }
        val fix = catalogId?.let { fixesProvider()[source to it] }
        if (fix != null && catalogId != null) {
            Timber.i("GameFixesRegistry: Applying fixes for game: $source $catalogId")
            resolvePaths(context, source, gameId)?.let { (installPath, installPathWindows) ->
                fix.apply(context, catalogId, installPath, installPathWindows, container)
            }
        }

        // LuaTools/SLS fixes (non-owned games) have no static entry. They record the
        // DLLs their fix shipped and must re-force them native on every launch, so the
        // override survives a container reset/recreation. No-op when no manifest exists.
        if (source == GameSource.STEAM) {
            runCatching { FixesRepository.reapplyDllOverrides(gameId.toInt(), container) }
                .onFailure { Timber.w(it, "GameFixesRegistry: LuaTools override reapply failed") }
        }
    }
